...
- (void)loadData {
    ...
    for (NSInteger i = 0; i < _datasourceImages.count; i++) {
        ...
        if (imageView.image != nil) {
            float imgWidth = (contentWidth*imageView.image.size.width)/imageView.image.size.height;
            if (imgWidth > contentWidth) {
                imgWidth = contentWidth;
            }
            imageView.frame= CGRectMake(contentWidth * i + (contentWidth-imgWidth)/2, 0, imgWidth, contentHeight);
        }
    }
